(function(root, factory){
	...
	rJS.contains = function(obj, target){
		if(obj == null){
			return false;
		}
		...
		return rJS.indexOf(obj, target) >= 0;
	};

	rJS.indexOf = function(array, item){
		if(array == null){
			return -1;
		}
	...
	rQuery.prototype.type = function(value){
		return this.prop('type', value);
	}

	rQuery.prototype.prop = function(prop, value){
	...
	// DOM取值赋值
	Rhythm.prototype.getValue = function(element){
		var self = this, element || self._element;
		var getters = {
			'INPUT': function(){
				var type = rQ.dom(element).type();
				if(rJS.contains(['text', 'password'], type)){
					return rQ.dom(element).val();
				}
	...
	Rhythm.prototype.setValue = function(){};
	...
	Rhythm.prototype.registerDependencies = function(){
		// this.registerBindings();
	};

	Rhythm.prototype.updateDependencies = function(selector){
	...
	var timeoutDOM = null, timeoutInput = null;
	...
	var eventInputChange = function(){
		...
		timeoutInput = setTimeout(function(){
			var element = rQ.dom(e.target).get(0);
			ry.dom(element).toStorage();
		}, ry.options.timeoutInput);
	};
	// 事件侦听列表
